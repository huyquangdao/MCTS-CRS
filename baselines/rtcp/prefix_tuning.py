...
class PrefixTuningTemplate(nn.Module):
    ...
    def generate_parameters(self) -> None:
        ...
        self.input_action_tokens = nn.Parameter(torch.arange(self.n_action_toks * self.n_actions).long(),
                                                requires_grad=False)  # to allow automatic devicing
        self.input_topic_tokens = nn.Parameter(torch.arange(self.n_topic_toks * self.n_topics).long(),
                                               requires_grad=False)  # to allow automatic devicing
    ...
    def compute_action_topic_prompt(self, batch):
        ...
        decoder_action_values = self.action_decoder_wte(
            self.input_action_tokens)  # self.n_action_toks * self.n_actions * n_embedd
        decoder_topic_values = self.topic_decoder_wte(
            self.input_topic_tokens)  # self.n_topic_toks * self.n_topics * n_embedd
        decoder_action_values = self.action_control_trans(decoder_action_values)
        decoder_topic_values = self.topic_control_trans(
            decoder_topic_values)  # (self.n_topic_toks * self.n_topics) * (self.n_decoder_layer * 2 * self.n_embd)
        b_action_values = []
        b_topic_values = []
        ...
        action_topic_prompt = torch.cat([b_action_values, b_topic_values],
                                        dim=1)  # bs x 4 x (self.n_decoder_layer * 2 * self.n_embd)

        #### IMPORTANT this code reshape the additional decoder tokens to the shape of the hidden vector computed by the PLM model.
        action_topic_prompt = action_topic_prompt.view(-1, self.n_action_toks + self.n_topic_toks,
                                                       self.match_n_decoder_layer * 2, self.match_n_head,
                                                       self.match_n_embd)
        #### appling drop out to the soft promots
        ...
        if 'attention_mask' in batch:
            ...
            if self.use_goal_topic:
                batch['attention_mask'] = torch.cat([torch.ones(
                    (batch_size, self.num_token + self.n_action_toks + self.n_topic_toks), dtype=am.dtype,
