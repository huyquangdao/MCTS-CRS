...
from eval.rtcp_online_eval import RTCPOnlineEval
...
def parse_args():
    ...
    # generation model
    parser.add_argument("--generation_plm_model", type=str)
...
if __name__ == '__main__':
    ...
    context_encoder = AutoModel.from_pretrained(args.policy_plm_model)
    path_encoder = AutoModel.from_pretrained(args.policy_plm_model)
    ...
    policy_model = load_model(policy_model, os.path.join(args.output_dir, policy_model_name))
    ...
    generation_plm = GPT2LMHeadModel.from_pretrained(args.generation_plm_model)
    ...
    generation_plm.resize_token_embeddings(len(generation_tokenizer))
    plm_config = GPT2Config.from_pretrained(args.generation_plm_model)
    ...
    response_model = PromptGPT2(plm=generation_plm, prefix_model=prefix_model, freeze_plm=args.freeze_plm)
    response_model = load_model(response_model, os.path.join(args.output_dir, generation_model_name))
    ...
    rtcp_online_eval = RTCPOnlineEval(
