...
def parse_args():
    ...
    parser.add_argument("--topic_outpath", type=str, help="Where to store the final model.")
    # model
...
if __name__ == '__main__':
    ...
    # load goal predictions
    dev_pred_topics = load_policy_results(os.path.join(args.topic_outpath, "dev_topic.txt"))
    test_pred_topics = load_policy_results(os.path.join(args.topic_outpath, "test_topic.txt"))
    ...
    dev_torch_dataset = UnimindTorchDataset(
        ...
        device=device,
        convert_example_to_feature=input_transformation_dict['response'],
        is_test=True,
        is_gen=True,
