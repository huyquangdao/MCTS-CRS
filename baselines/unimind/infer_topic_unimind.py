...
from dataset.data_utils import load_binary_file, load_policy_results, merge_predictions, save_knowledge_results
...
if __name__ == '__main__':
    ...
    dev_torch_dataset = UnimindTorchDataset(
        ...
        device=device,
        convert_example_to_feature=input_transformation_dict['topic'],
        is_test=True,
        is_gen=True,
    ...
    logger.info({"topic_accuracy": goal_acc})
    if run:
        run.log({"topic_accuracy": goal_acc})
    ...
    # post processing the pred topics and label topic
    ...
    save_knowledge_results(valid_preds, os.path.join(args.output_dir, "dev_topic_preds.txt"))
    save_knowledge_results(test_preds, os.path.join(args.output_dir, "test_topic_preds.txt"))
